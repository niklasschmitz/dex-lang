...
module Syntax (
    ...
    ImpModule (..), ImpBlock (..), ImpFunctionDef (..), ImpDecl (..),
    ...
    DataDef (..), DataConDef (..),UTyConDef (..), UDataConDefs (..),
    Nest (..), toNest,
    ...
    pattern NestOne, pattern NewTypeCon,
    -- pattern BinderAnn,
...
data Atom n =
 ...
 | Lam (LamExpr n)
 | Pi  (PiType  n)
 | DataCon (DataDef n) [Atom n] Int [Atom n]
 | TypeCon (DataDef n) [Atom n]
...
data DataDef n = DataDef (Name n) (Abs (Nest Binder) (ListH DataConDef) n)  deriving (Show, Eq, Generic)
data DataConDef n = DataConDef (Name n) (EmptyNest Binder n)   deriving (Show, Generic)
...
data PartialBindings n l = PartialBindings

data Module n l where
  Module :: IRVariant -> Nest Decl n l' -> PartialBindings l' l -> Module n l
...
data UTyConDef    n l = UTyConDef    (UBinder     n l) (EmptyNest UAnnBinder n)
data UDataConDefs n l = UDataConDefs (UBinderList n l) [EmptyNest UAnnBinder n]
data UDecl n l where
 ULet :: LetAnn ->  UPatAnn n l -> UExpr n -> UDecl n l
 UData :: UTyConDef n tc -> UDataConDefs tc dcs -> UDecl n dcs
 -- superclasses, dictionary type constructor, types of methods
 UInterface :: [UType n] -> UTyConDef n l' ->  Nest UAnnBinder l' l -> UDecl n l
 -- name, args (conditions), dictionary type, method definitions
 UInstance :: Maybe (UVar n) -> Nest UPatAnnArrow n l' -> UType l' -> [UMethodDef l'] -> UDecl n n
...
data UModule n l = UModule (Nest UDecl n l)  deriving (Show)
...
data SourceBlock' = RunModule (UModule SourceNS SourceNS)
                  | Command CmdName (Name SourceNS, UModule SourceNS SourceNS)
...
data ImpModule n where
  -- local function names, local function definitions, main function definition
  ImpModule :: BinderListP IFunTypeH n l -> [ImpFunctionDef l] -> ImpFunctionDef l
            -> ImpModule n

data ImpFunctionDef n = ImpFunction (Abs IBinderList ImpBlock n)
                      | FFIFunction String
                        deriving (Show)
data ImpBlock n = ImpBlock (Abs (Nest ImpDecl) (ListH IExpr) n)  deriving (Show)
...
data BinderInfo n =
 ...
 | DataBoundDataCon (DataDef n) Int
 | DataBoundTypeCon (DataDef n)
 | PiBound
 | UnknownBinder
   deriving (Show, Generic)

type SubstEnv = Env Atom
type Bindings n = ScopeP (PairH Type BinderInfo) n
type Scope n = Bindings n  -- when we only care about the names, not the payloads
...
pattern NewTypeCon con ty = [DataConDef con (Abs (NestOne (Ignore ty)) UnitH)]

data ClassDictDef n = ClassDictDef
  { cddName         :: Name n
  , cddConName      :: LabeledItems (Type n)
  , cddSuperclasses :: LabeledItems (Type n)
  , cddMethods      :: [DataConDef n] }
